...
<body>
    ...
    <div>
        <section class="home-section">
            <nav>
                <form action="{{ route('admin.order.search') }}" method="GET" style="padding-top: 10px;">
                    <input type="text" name="id" class="order_quantity" placeholder="Search by user id">
                    <button type="submit" class="order_btn">Filter</button>
                </form>
                <table class="table">
                    <thead>
                        <tr>
                        ...
                        @if ($orders->count() > 0)
                            @foreach ($orders as $category)
                                <tr>
                                    ...
                                    <td class="btns">
                                        <form action="{{ route('admin.show_order', $category->id) }}">
                                            <button class="btn1" style="color: #FFFFFF" type="submit">show</button>
                                        </form>
                                        <form method="POST"
                                            action="{{ route('admin.destroy_order', $category->id) }}">
                                            @method('DELETE')
                                            @csrf
                                            <button class="btn2" style="color: #FFFFFF" type="submit">delete</button>
                                        </form>
                                        <form action="{{ route('admin.update_order', $category->id) }}">
